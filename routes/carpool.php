...
Route::prefix('passaggi')->name('carpool.')->middleware(CarpoolPrivacy::class)->group(function (): void {
    ...
    Route::middleware('auth')->group(function (): void {
        ...
        Route::post('/richieste/{rideRequest}/recensione/{action}', [RideReviewsController::class, 'change'])->whereNumber('rideRequest')->whereIn('action', ['confirm', 'save', 'remove'])->middleware('throttle:carpool-write')->name('review.action');
        ...
        Route::post('/azioni/{action}', [C::class, 'action'])->middleware('throttle:carpool-write')->name('action');
        Route::post('/ricerche/{action}', [C::class, 'discovery'])->middleware('throttle:carpool-write')->name('discovery');
        ...
        Route::post('/assistenza/{case}', [C::class, 'reply'])->whereNumber('case')->middleware('throttle:carpool-report')->name('case.reply');
    });
});

Route::post('/admin-community/cases/{case}/evidence', CarpoolEvidenceController::class)
    ->whereNumber('case')->middleware(['auth', CarpoolPrivacy::class, 'throttle:carpool-report'])->name('carpool.evidence');
