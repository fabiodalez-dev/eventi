...
Route::prefix('v1/carpool')->middleware(['auth:sanctum', CarpoolPrivacy::class])->group(function (): void {
    ...
    Route::post('/requests/{rideRequest}/review/{action}', [RideReviewsController::class, 'change'])->whereNumber('rideRequest')->whereIn('action', ['confirm', 'save', 'remove'])->middleware('throttle:carpool-write');
    ...
    Route::post('/actions/{action}', [C::class, 'action'])->middleware('throttle:carpool-write');
    Route::post('/discovery/{action}', [C::class, 'discovery'])->middleware('throttle:carpool-write');
    ...
    Route::post('/cases/{case}', [C::class, 'reply'])->whereNumber('case')->middleware('throttle:carpool-report');
});
