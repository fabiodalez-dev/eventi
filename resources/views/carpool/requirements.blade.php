...
@else
<div class="my-8 space-y-5"><p>{{ __($access['whatsapp_exempt'] && !$access['whatsapp_verified'] ? 'community.whatsapp.exempt' : 'carpool.verified') }}</p><p>{{ __('carpool.adult_status') }} · {{ \Carbon\CarbonImmutable::parse($access['adult_declared_at'])->format('d/m/Y') }}</p><x-button :href="route('carpool.index')">{{ __('carpool.mine') }}</x-button></div>
