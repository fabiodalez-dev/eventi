<x-layouts.app :meta="$meta">
    ...
    <form data-community-form method="post" action="{{ route('community.publish', $saved->occurrence_id) }}" class="space-y-6">@csrf @method('PUT')
        ...
        @php($visibility = $verified ? old('visibility', $visibility) : 'private')
        <fieldset class="space-y-3"><legend class="font-semibold">{{ __('community.save_privacy') }}</legend>
            ...
            <label class="flex min-h-11 items-center gap-3"><input type="radio" name="visibility" value="public" @disabled(! $verified) @checked($visibility === 'public')>{{ __('community.public') }}</label>
        </fieldset>
        @if($verified)
