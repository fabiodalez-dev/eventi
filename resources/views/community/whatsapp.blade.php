    ...
    <div class="mx-auto max-w-xl">
        ...
        @if($verified)
            ...
            <details class="mt-10 border-t border-line pt-5"><summary class="cursor-pointer">{{ __('community.whatsapp.revoke') }}</summary>
                ...
                <form method="post" action="{{ route('community.whatsapp.revoke') }}">@csrf @method('DELETE')<x-button type="submit" variant="secondary">{{ __('community.whatsapp.revoke') }}</x-button></form>
            </details>
        @elseif(!$available)
            <p role="status" class="my-8 border-l-4 border-brand bg-surface p-5">{{ __('community.whatsapp.unavailable') }}</p>
        @else
            @if($challenge_id)
                <form method="post" action="{{ route('community.whatsapp.confirm') }}" class="my-8 space-y-4">@csrf
            ...
            @endif
            <form method="post" action="{{ route('community.whatsapp.send') }}" class="my-8 space-y-4">@csrf
        ...
        @endif
        @unless($verified)<form method="post" action="{{ route('community.whatsapp.skip') }}" class="mt-6">@csrf<button class="min-h-11 underline">{{ __('community.whatsapp.later') }}</button></form>@endunless
    </div>
</x-layouts.app>
