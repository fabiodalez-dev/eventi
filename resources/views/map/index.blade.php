...
<x-layouts.app :meta="$meta" :wide="true">
    ...
    <div data-event-browser data-filter-fade data-result-count="{{ $occurrences->count() }}" class="grid items-start gap-0.5 bg-line lg:[grid-template-columns:minmax(240px,300px)_minmax(0,1fr)]">
        <aside class="order-2 flex flex-col gap-6 overflow-y-auto bg-canvas p-[clamp(1rem,1.6vw,1.375rem)] lg:order-1 lg:sticky lg:top-header lg:max-h-below-header">
            ...
            <div data-filter-transition class="flex flex-col gap-6">
                <x-filter-bar
                    :counts="$facetCounts"
                    :filters="$filters"
        ...
        <section data-filter-transition aria-label="{{ __('map.label') }}" class="order-1 flex flex-col bg-canvas lg:order-2 lg:sticky lg:top-header lg:h-below-header">
            <x-events-map
                :city="$city"
                :filters="$filters"
                :payload="$payload"
    ...
    @if ($occurrences->isNotEmpty())
        <section aria-labelledby="mappa-elenco" class="map-results relative isolate z-10 order-3 bg-canvas lg:col-span-2">
            <div data-filter-transition>
