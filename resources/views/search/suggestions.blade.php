@if (mb_strlen($term) >= 3)
    @foreach (['events' => $events, 'venues' => $venues, 'tags' => $tags] as $group => $items)
        @if ($items->isNotEmpty())
            <section class="border-b border-line p-3">
                <h2 class="mb-1 font-display text-xs font-extrabold uppercase tracking-wider text-accent">{{ __('search.groups.'.$group) }}</h2>
                <ul>
                    @foreach ($items as $item)
                        <li>
                            <a class="block px-2 py-2 text-sm text-ink hover:bg-surface focus:bg-surface focus:outline-2 focus:outline-accent" href="{{ match ($group) {
                                'events' => route('events.show', ['slug' => $item->event->slug]),
                                ...
                                @if ($group === 'events')
                                    <span class="block text-xs text-ink-muted">{{ $item->event->venue?->name ?? $city->name }} · {{ $item->starts_at->timezone($city->timezone)->format('d/m') }}@unless ($item->is_all_day) {{ $item->starts_at->timezone($city->timezone)->format('H:i') }}@endunless</span>
                                @endif
                            </a>
                        </li>
                    @endforeach
        ...
        @endif
    @endforeach
    @if ($events->isEmpty() && $venues->isEmpty() && $tags->isEmpty())
