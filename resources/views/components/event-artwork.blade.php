...
<div {{ $attributes->class('event-poster-frame') }}>
    @if ($artwork !== null)
        <x-media-image :set="$artwork" :alt="__('events.card.poster_alt', ['title' => $event->title])" width="600" height="800" :sizes="$sizes" :eager="$eager" :color="true" data-poster-reveal class="poster-reveal" :show-placeholder="$eager" />
    @else
