@props(['rows', 'name', 'metric', 'label', 'secondary' => null, 'link' => null])
...
<div class="ad-bars" data-analytics-bars aria-label="{{ $label }}">
    @forelse ($rows as $row)
        <div class="ad-bar-row">
            <div><span>@if ($link)<a class="ad-drilldown" href="{{ $link($row) }}">{{ $row[$name] }}</a>@else{{ $row[$name] }}@endif</span><strong>{{ number_format($row[$metric] ?? 0, 0, ',', '.') }}@if ($secondary !== null)<small>{{ $secondary['label'] }}: {{ number_format($row[$secondary['key']] ?? 0, 0, ',', '.') }}</small>@endif</strong></div>
