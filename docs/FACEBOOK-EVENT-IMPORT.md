...
La richiesta identifica l’importatore come `inCitta-event-import/1.0 (+https://eventi.fabiodalez.it)` e usa `locale=it_IT`, `Accept: text/html,application/xhtml+xml` e `DPR: 3`. Le risposte con User-Agent generico o imitazione del browser avevano restituito anteprime o errori: non bastava cambiare linguaggio. La risposta HTTP corretta contiene i frammenti JSON completi. Una sola anteprima OpenGraph rimane un errore, mai una descrizione sostitutiva.

Il download e la verifica MIME/dimensioni delle dieci foto sono stati eseguiti dal servizio applicativo. La richiesta usa `DPR: 3` per ricevere immagini ad alta risoluzione: tutte le dieci foto coincidono con le dimensioni della variante originale dichiarata da Facebook, da 720 a 2048 pixel di larghezza. TEATRO BRESCI è 2048×1072, SICK TAMBURO 1200×628 e Day Bau Day 2048×916. Il parser sceglie la variante disponibile più grande della stessa copertina, escludendo anteprime sfocate e dimensioni dichiarate oltre il limite di 40 milioni di pixel. Nessun ingrandimento artificiale, modifica degli URL firmati o accesso alla pagina privata della foto. I byte scaricati restano soggetti ai controlli di dimensione, MIME e pixel.
