...
compilati dal runner. Il server dispone di Node tramite nvm; la build degli asset rimane in CI. L’import Facebook usa HTTP in PHP e il parser Node, senza Chromium: al cambio di server riconfigurare il runtime come descritto in [FACEBOOK-EVENT-IMPORT](FACEBOOK-EVENT-IMPORT.md) e verificare `php artisan facebook:check`.
