...
**Chiave dell'impronta.** L'impronta del numero usa soltanto `WHATSAPP_PHONE_HASH_KEY`, senza ripiego su `APP_KEY`: senza la chiave la verifica WhatsApp risulta non disponibile. L'installer la genera per le installazioni nuove; per i server esistenti la procedura è nel runbook. Una rotazione della chiave cambierebbe tutte le impronte e riaprirebbe i numeri già usati, quindi non si fa a mano (issue #103): `php artisan community:rehash-phones` ricalcola le impronte di utenti e richieste dai numeri cifrati, con la chiave nuova dalla configurazione e la vecchia soltanto dall'ambiente del processo (`WHATSAPP_PHONE_HASH_PREVIOUS_KEY`). Lavora sotto il lock delle richieste di codice e in una sola transazione, e non scrive nulla se una riga non torna con nessuna delle due chiavi, se due account finirebbero sulla stessa impronta o se ci sono impronte orfane di sospesi cancellati, che si eliminano solo con `--release-orphans`. `--dry-run` mostra il resoconto senza scrivere; il comando non stampa mai numeri, impronte o chiavi. La procedura è nel runbook.

**Sospensione e cancellazione.** Se un account sospeso viene cancellato restano l'impronta del numero e la data della sospensione, così lo stesso numero non torna con una nuova iscrizione; il numero invece viene cancellato. L'informativa lo dice. Dopo ventiquattro mesi dalla sospensione (`suspended_fingerprint_retention_months` in `config/community.php`) impronta e data della sospensione vengono eliminate da `community:prune-fingerprints`, che gira ogni notte e tocca solo account cancellati: un account attivo sospeso resta sospeso (issue #104). Il periodo è nell'informativa, portato nelle pagine già pubblicate dalla migrazione `2026_09_19_100000_community_fingerprint_retention_period`. Chi è sospeso non può revocare da sé la verifica, perché libererebbe il numero; lo staff sì.
